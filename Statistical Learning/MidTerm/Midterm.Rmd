...
When you download the data from the source, you get a directory called R. In this directory there are instructions and code on how to make the given metadata as a R data frame. This code was quite outdated so we have updated it and let it in the Annex. It is run just below with the name "MD_EES_2018.R".
...
For the OECD[^2], the lower class in Spain is the one whose income is below 75% of the national median income; the middle class is between 75% and 200% of the median, and the upper class is the one above 200% of the median. 
...
We ensure this is well done by our caret models by using the [Kappa metric](https://en.wikipedia.org/wiki/Cohen's_kappa) for the model comparison (Kappa metric tends to take into account model imbalances).
...
z <- ggplot(lr_imp, aes(x = Feature, y = Importance, fill = Feature)) +
  geom_bar(stat = "identity") +
  labs(x = "Feature", y = "Importance") +
  theme(axis.text.y = element_text(angle = -45, vjust = 0.5, size = 0.2),  # Tamaño de la letra más pequeño
