...
kcvRidge <- cv.glmnet(x = as.matrix(train$px), y = train$digit, alpha = 0, standardize = FALSE, 
                      family = "binomial", nfolds = 10, lambda = lambdaGrid)
```
...
```{r ridge2}
ridgeMod <- glmnet(x = as.matrix(train$px), y = train$digit, alpha = 0, 
                   family = "binomial", standardize = FALSE ,lambda = 83.64)
...
```{r plot, echo=FALSE, fig.height=3.5, fig.width=3.5, fig.align='center'}
plot(coef(ridgeMod)[-1])
```
It seems clear how the central pixels tend to have bigger coefficients than the first ones. This is because of the number being drawn in the centre of the grid.
...
Note how the previous code takes the base factor as de smallest digit. This makes sense since the datasets are created that way and also `class_prediction` loop is working the same way.
