...
The aim of this document is to solve the four tasks proposed as deliverables for the course *High Performance Computing for Data Science* of the *MSc Statistics for Data Science* of the *UC3M*. The assignment focuses on the problem of chain matrix multiplication and associativity. 
...
However determining the best parenthesization is not an easy task, since the number of possible parenthesizations of a set of matrices is exponential^[The number of ways to parenthesize an expression of $n+1$ terms is given by the $n^{th}$ Catalan number $C_n=\frac{1}{n+1}\binom{2n}{n}$].

Several solutions exist, and in this document we will be exploring **[INSERT NAME HERE]**
...
**AQUI PONER LAS TECNOLOGIAS Y LIBRERIAS QUE VAMOS USANDO QUE TENGAN QUE VER CON LA ASIGNATURA**

# Task 1

Given a set of matrices $\mathcal{A}_{50\times15}$, $\mathcal{B}_{15\times70}$, $\mathcal{C}_{70\times10}$, $\mathcal{D}_{10\times50}$, $\mathcal{E}_{50\times2}$; the naive multiplication  $\mathcal{A}\mathcal{B}\mathcal{C}\mathcal{D}\mathcal{E}$ takes $50*15*70+50*70*10+50*10*50+50*2*50$=`r 50*15*70+50*70*10+50*10*50+50*2*50` operations.

Lets consider some different parenthesization, for example $\mathcal{A}(\mathcal{B}\mathcal{C})\mathcal{D}\mathcal{E}$. The number of operations involved are now $15*70*10+50*15*10+50*10*50+50*50*2$=`r 15*70*10+50*15*10+50*10*50+50*50*2`. This is a reduction of nearly `r (1-(15*70*10+50*15*10+50*10*50+50*50*2)/(50*15*70+50*70*10+50*10*50+50*2*50))*100`$%$ of the complexity compared to the naive approach. 

Furthermore, lets consider now the order  $\mathcal{A}((\mathcal{B}\mathcal{C})(\mathcal{D}\mathcal{E))}$. Now the number of operations involved are $15*70*10+10*50*2+10*50*2+50*15*2=$`r`15*70*10+10*50*2+10*50*2+50*15*2. This time the reduction is `r (1-(15*70*10+10*50*2+10*50*2+50*15*2)/(50*15*70+50*70*10+50*10*50+50*2*50))*100` compared to the standard approach but also `r (1-(15*70*10+10*50*2+10*50*2+50*15*2)/(15*70*10+50*15*10+50*10*50+50*50*2))*100` compared to the second approach. 

This can be visualize in: 

**MIRAR COMO HACER GRAFICOS QUE SEAN COMPELING**
...
FROM THIS PART THE CODE IS FOR CONSULTING THE SYNTAX
# Introduction
